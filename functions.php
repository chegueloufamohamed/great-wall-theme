...
function great_wall_scripts() {
	...
	wp_enqueue_style( 'great-wall-styles', get_template_directory_uri() . '/assets/css/style.css', array(), '3.2.7' );
	...
	wp_enqueue_script( 'great-wall-js', get_template_directory_uri() . '/assets/js/main.js', array(), '2.2.2', true );
...
/**
 * Remove Default WooCommerce Sale Flash Badges
 */
...
function great_wall_add_free_shipping_fixing_badge() {
	...
	echo '<div class="free-shipping-fixing-badge">' . esc_html__( 'Free Shipping and Fixing: Dubai, Sharjah, Ajman', 'great-wall-theme' ) . '</div>';
	echo '</div>';
}
 ...
 */
add_action( 'great_wall_after_product_gallery', 'great_wall_display_single_product_variations_links', 50 );
function great_wall_display_single_product_variations_links() {
			...
			<?php foreach ( $linked_products as $lp ) : 
				...
				<a href="<?php echo esc_url( get_permalink( $lp->get_id() ) ); ?>" class="variation-link-btn<?php echo $is_current ? ' active' : ''; ?>">
					...
					<div class="variation-link-content">
						...
						<div class="variation-link-meta-row">
							<?php if ( ! empty( $dimensions ) ) : ?>
								<span class="variation-link-dimensions"><?php echo esc_html( $dimensions ); ?></span>
								<span class="variation-link-separator">&bull;</span>
							<?php endif; ?>
	...
	<script>
		jQuery(document).ready(function($) {
			function moveSwatchesToBottom() {
				var $gallery = $('.woocommerce-product-gallery');
				var $swatches = $('.product-variation-links');
				if ($gallery.length && $swatches.length) {
					if (!$swatches.is(':last-child')) {
						$gallery.append($swatches);
					}
				}
			}
			$(document).on('wc-product-gallery-after-init', moveSwatchesToBottom);
			moveSwatchesToBottom();
			setTimeout(moveSwatchesToBottom, 150);
			setTimeout(moveSwatchesToBottom, 600);
		});
	</script>
	<?php
}
